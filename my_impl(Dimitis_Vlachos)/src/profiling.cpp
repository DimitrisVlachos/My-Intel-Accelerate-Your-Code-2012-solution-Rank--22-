...
static inline const f64 s_to_m(const f64& s) {
    return ((s + 0.00001) / 60.f);
}
 
static scope_info_t* grab_scope_entry(const std::string& s) {
    ...
    uint32_t old_size = scope_info.size();
...
void profiler_update(const std::string& scope_desc,const f64& delta) {
